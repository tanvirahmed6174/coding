...
def get_fwhm(ws, s21s_abs_tilted_shifted, kappa_threshold, w0_ind_list, w0_mask_list, i):
    # calculate half max
    w0 = ws[w0_ind_list[i]]
...
def graphical_guess(ws, s21s):
    ...
    fwhm = get_fwhm(ws, s21s_abs_tilted_shifted, kappa_threshold, w0_ind_list, w0_mask_list, 0)
...
def graphical_guess_coupled(ws, s21s, nr_dips=2):
    ...
    # Compute FWHMs
    fwhms = [get_fwhm(ws, s21s_abs_tilted_shifted, kappa_threshold, w0_ind_list, w0_mask_list, i) for i in range(nr_dips)]
...
def fit_resonance_coupled(
    ...
    s21s: Optional[np.ndarray] = None, 
    manual_graph_guess: Optional[dict] = None,
    return_full: bool = False,   # new flag
    verbose: bool = True         # toggle printing fit_report
) -> Tuple[dict, np.ndarray] | Tuple[lmfit.model.ModelResult, dict, np.ndarray]:
    """
    Fit function for coupled mode, parallel to fit_resonance.
    ...
    s21s : array
        Complex S21 values.
    manual_graph_guess : dict, optional
        Manual override for graphical guess.
    return_full : bool
        If True, return the full lmfit result object as well.
    verbose : bool
        If True, print the fit_report.
    
    Returns
    -------
    If return_full is False:
        (fit_results_dict, s21_fit_trace)
    If return_full is True:
        (lmfit_result, fit_results_dict, s21_fit_trace)
    """
    graph_guess = graphical_guess_coupled(ws / w_scale, s21s)  # type: ignore
    if manual_graph_guess is not None:
        graph_guess.update(**manual_graph_guess)
    ...
    if return_full and result is not None:
        return result, fit_results_dict, s21_fit_trace
    else:
        return fit_results_dict, s21_fit_trace
